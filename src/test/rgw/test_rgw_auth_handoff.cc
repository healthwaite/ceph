 ...
 */
static std::optional<std::string> get_aws_v4_hash(std::string string_to_sign, std::string access_key_id, std::string secret_key, std::string authorization)
{
...
static rgw::HandoffVerifyResult verify_by_func(const DoutPrefixProvider* dpp, const std::string& request_json, ceph::bufferlist* resp_bl, [[maybe_unused]] optional_yield y)
{
  ...
  auto signature = get_aws_v4_hash(string_to_sign, access_key_id, secret, authorization);
...
TEST(HandoffMeta, Sig)
{
  for (const auto& t : tests) {
    ...
    auto sig = get_aws_v4_hash(t.ss_base64, t.access_key, (*info).secret, t.authorization);
    ASSERT_TRUE(sig);
  }
}
...
// XXX this should work, we should support V2 signatures.
...
// Test working signatures with the test function above.
