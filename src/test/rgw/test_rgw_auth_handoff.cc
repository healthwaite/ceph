...
static rgw::HandoffVerifyResult verify_by_func(const DoutPrefixProvider* dpp, const std::string& request_json, ceph::bufferlist* resp_bl, [[maybe_unused]] optional_yield y)
{
  ...
  auto signature = verify_aws_signature(string_to_sign, access_key_id, secret, authorization);
  std::string message;
  if (signature) {
    message = "OK";
  } else {
...
int main(int argc, char** argv)
{
  ...
  g_ceph_context->_conf->subsys.set_log_level(ceph_subsys_rgw, 20); // XXX high
